...
int main(void)
{
  ...
  while (1)
  {
	  while(TIM3_Ch_DCVAL < 60000) {
	  		 TIM3_Ch_DCVAL += (50);
	  		 TIM3->CCR1 = TIM3_Ch_DCVAL;
	  		 HAL_Delay(1);
	  	 }

	  	 HAL_Delay(5000);

	  	 while(TIM3_Ch_DCVAL > 0) {
	  		TIM3_Ch_DCVAL -= (50);
	  		TIM3->CCR1 = TIM3_Ch_DCVAL;
	  		HAL_Delay(1);
	  	 }
	  	 ...
	  	 sprintf((char*)txd_msg_buffer, "RPM Tick Count: %d \r \n", rpm_tick_count);
